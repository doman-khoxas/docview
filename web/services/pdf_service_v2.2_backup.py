...
def do_ocr(doc_id: str, language: str = "eng") -> dict:
    ...
    ocr_path = engine.ocr_document(language=language)
...
def ocr_oneshot(file_storage, language: str = "eng") -> tuple[str, str]:
    """Upload → OCR → return path to OCR'd file. For stateless use."""
    doc_id = uuid.uuid4().hex[:12]
    ...
    if os.path.exists(save_path) and save_path != ocr_path:
        os.unlink(save_path)
